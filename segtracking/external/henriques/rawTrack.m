function bb=rawTrack(sceneInfo,initBox,prevModel,allim)
show_output=true;
show_output=false;
mid=[]; mt=[];
...
npos=numel(y);
% ts_half=target_sz./2;
% ts_half=ts_half'; ts_half=repmat(ts_half,1,npos);

allinfo(1).pos = repmat(pos',1,npos);
% round([initBox(2), initBox(1)] + target_sz./2);
...
%         npos=numel(response);
        allinfo(frame).response=response;
        [allr, allc]=ind2sub(size(response),1:npos);
        allrc=[allr;allc];
        allpos=repmat(pos',1,npos);
        allsz=floor(repmat(sz'/2,1,npos));
        
%         pause
%         size(allinfo(frame).pos)
%         size(allsz)
%         size(allrc)
        allinfo(frame).pos = allinfo(frame-1).pos - allsz + allrc;
%         allinfo(frame).pos = allpos - allsz + allrc;
%         pause
        %         response(1:15)
    ...
    if frame == 1,  %first frame, train with a single image
        if nargin>=3 && ~isempty(prevModel)
            alphaf = prevModel.alphaf;
            z = prevModel.z;
        else
            alphaf = new_alphaf;
            z = x;
    ...
    if frame==1
        allinfo(frame).rect=rect_position;
    else
        allinfo(frame).rect = [ ...
            (allinfo(frame).pos(2,:)-target_sz(2)/2)', ...
            (allinfo(frame).pos(1,:)-target_sz(1)/2)', ...
            repmat(target_sz([2,1]),npos,1)];
    end    
