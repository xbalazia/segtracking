function chns = chnsCompute_my( I, varargin)
...
if( ~isfield(pChns,'complete') || pChns.complete~=1 || isempty(I) )
  ...
  pChns = getPrmDflt(varargin,{'shrink',4,'pColor',{},'pGradMag',{},...
    'pGradHist',{},'pCustom',p,'complete',1,'cbin', 5,'train',1, 'NNRadius', 1,'shrinkadd',[]},1);
...
shrink=pChns.shrink; [h,w,nch]=size(I); cr=mod([h w],shrink);
...
Hist = mexgrid_sum(double(I), h,w,nch,shrink);% for haar features;
if(p.enabled), chns=addChn(chns,single(Hist),nm,pChns.pColor,0,h,w); end% for haar features;
...
if( p.enabled )
    Hist = mexgrid_sum(double(M), h,w,1,shrink);% for haar features;
    if(p.enabled), chns=addChn(chns,single(Hist),nm,p,0,h,w); end% for haar features;
end
