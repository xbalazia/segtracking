function pyramid = chnsPyramid_my( I, varargin )
...
if( ~isfield(p,'complete') || p.complete~=1 || isempty(I) )
  ...
  p=getPrmDflt(varargin,dfs,1); chns=chnsCompute_my([],p.pChns);
...
for i=isR
  ...
  chns=chnsCompute_my(I1,pChns); info=chns.info;
...
if(any(pad)), for i=1:nScales, for j=1:nTypes
      data{i,j}=imPad(data{i,j},pad/shrink,info(j).padWith); end; end; end %for cell features:pad/shrink; for pixel features:pad
